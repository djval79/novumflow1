...
import { MOCK_FORM_TEMPLATES, MOCK_FORM_SUBMISSIONS } from '../services/mockData';
...
const FormsPage: React.FC = () => {
  ...
  const handleSubmitAudit = () => {
    ...
    const newSubmission: FormSubmission = {
      ...
      answers: formAnswers,
      status: 'Submitted' // Logic could flag issues automatically here
    };
       ...
       {templates.map(tpl => (
          <div key={tpl.id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
             <div>
                ...
                <p className="text-xs text-slate-500">{tpl.questions.length} Questions • Created by {tpl.createdBy}</p>
             </div>
             <button 
          ...
          <tbody className="divide-y divide-slate-100">
             {submissions.map(sub => (
                <tr key={sub.id} className="hover:bg-slate-50">
                   ...
                   <td className="px-6 py-4 text-slate-600">{sub.submittedAt}</td>
                   <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase flex items-center w-fit gap-1 ${
                         sub.status === 'Flagged' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                      }`}>
                         ...
                         {q.type === 'YesNo' && (
                            <div className="flex gap-4">
                               <button 
                                  ...
                                  className={`flex-1 py-3 rounded-lg font-bold border transition-colors ${formAnswers[q.id] === true ? 'bg-green-600 text-white border-green-600' : 'bg-white text-slate-600 border-slate-300 hover:bg-slate-50'}`}
                               >
                                  Yes / Pass
                               </button>
                               <button 
                                  onClick={() => updateAnswer(q.id, false)}
                                  className={`flex-1 py-3 rounded-lg font-bold border transition-colors ${formAnswers[q.id] === false ? 'bg-red-600 text-white border-red-600' : 'bg-white text-slate-600 border-slate-300 hover:bg-slate-50'}`}
                               >
                                  No / Fail
                               </button>
                            </div>
                            ...
                            <div className="flex justify-between gap-2">
                               {[1,2,3,4,5].map(num => (
                                  <button 
                                     ...
                                     className={`flex-1 py-3 rounded-lg font-bold border transition-colors ${formAnswers[q.id] === num ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-slate-600 border-slate-300 hover:bg-slate-50'}`}
                                  >
                                     {num}
                                  </button>
                               ))}
