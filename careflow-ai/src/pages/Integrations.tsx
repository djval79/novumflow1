...
const Integrations: React.FC = () => {
   ...
   const toggleApp = (id: string, name: string) => {
      ...
      setApps(apps.map(app => {
         if (app.id === id) {
            return { ...app, status: newStatus, lastSync: 'Just now' };
         }
         return app;
      }));
      ...
      <div className="max-w-7xl mx-auto space-y-12 animate-in fade-in duration-700 pb-12">
         ...
         {activeTab === 'apps' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                  ...
                  <div key={app.id} className="bg-white p-12 rounded-[4rem] border border-slate-100 shadow-2xl flex flex-col justify-between h-[380px] group transition-all hover:shadow-primary-500/10 relative overflow-hidden">
                     ...
                     <div className="flex justify-between items-start relative z-10">
                        <div className="flex items-center gap-6">
                           <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center text-3xl font-black text-white shadow-2xl transform group-hover:scale-110 transition-transform duration-500
                        ${app.name.includes('Xero') ? 'bg-blue-500' :
                                 app.name.includes('Sage') ? 'bg-emerald-600' :
                                    app.name.includes('Slack') ? 'bg-purple-600' : 'bg-rose-600'
                              }
                      `}>
                              {app.icon}
                           </div>
                     ...
                     <div className="space-y-6 relative z-10 bg-slate-50/50 p-8 rounded-[2.5rem] border border-slate-50">
                        ...
                        <div className="flex items-center justify-between">
                           ...
                           <span className="text-[11px] font-black text-slate-900 uppercase tracking-tight">{app.lastSync}</span>
                        </div>
                     </div>
               ...
               <button
                  onClick={() => toast.info('Satellite Directory access restricted to Tier 1 Admins')}
               ...
               >
                  ...
                  <span className="font-black uppercase tracking-[0.5em] text-[12px] relative z-10">Expand Satellite Directory</span>
               </button>
            </div>
         )}
         ...
         {activeTab === 'webhooks' && (
            <div className="bg-white rounded-[4.5rem] border border-slate-100 shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-10 duration-700">
               <div className="px-12 py-12 border-b border-slate-50 bg-slate-50/20 flex flex-col md:flex-row justify-between items-center gap-10">
                  ...
                  <button
                     onClick={() => toast.info('Integrations restricted. Please contact systems architect.')}
