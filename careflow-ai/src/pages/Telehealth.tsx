...
import { 
  ...
  Sparkles, Volume2, Loader2, AlertCircle, Clock
} from 'lucide-react';
...
const Telehealth: React.FC = () => {
  ...
  const endAiSession = () => {
    ...
    // Note: There isn't a direct close method exposed easily on the promise result in this snippet context, 
    // but mostly we rely on closing the stream and context to stop interaction.
    setIsLive(false);
    setAiStatus('Disconnected');
    setVolumeLevel(0);
  };
  ...
  function createBlob(data: Float32Array): Blob {
    ...
    // Simple base64 encode for the blob data string (simplified for this environment)
    let binary = '';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             {MOCK_SESSIONS.map(session => (
                <div key={session.id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                   <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            ...
                            <p className="text-xs text-slate-500">with {session.hostName}</p>
                         </div>
                      </div>
                   ...
                   <button className="w-full py-3 bg-primary-600 text-white font-bold rounded-lg hover:bg-primary-700 shadow-lg shadow-primary-900/20 flex items-center justify-center gap-2">
